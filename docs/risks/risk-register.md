...
| R-008 | Users may mistake non-executable plans for runnable fixes. | Medium | High | Every plan marks `approval_required: true`; `execution_enabled` mirrors the contract gate; the Execute button runs only exact low-risk guarded actions and shows blocked reasons for everything else. | codex session | Open |
...
| R-010 | Privileged or broader modifying maintenance actions could raise the project risk tier. | Medium | High | Guarded execution is limited to low-risk, reversible, non-privileged catalog actions. Privileged, destructive, placeholder, medium-risk, or unsupported commands remain blocked and require another governance reassessment before enablement. | Adam Goodwin | Open |
