...
| R-012 | Users may assume visible action contracts mean commands can be run. | Medium | High | Contracts show `execution_enabled: false`, gate reasons are visible in the Approval Queue, `/api/action-run` records blocked results, and docs require governance reassessment before execution. | codex session | Open |
