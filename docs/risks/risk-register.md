...
| --- | --- | --- | --- | --- | --- | --- |
| R-001 | External governance preflight can fail if `GOVERNANCE_HOME` is unset or the governance repo moves. | Low | Medium | `GOVERNANCE_HOME` configured in shell startup; verified path `/home/adamgoodwin/code/Rules of Development and Deployment`; preflight rerun successfully. | Adam Goodwin | Mitigated |
